...
int 
papi_cookie_handler(request_rec * r, papi_dir_config * d)
{
...
static int 
papi_cookie_test(request_rec * r, papi_dir_config * d)
{
	int             init = UNSET;
...
static int 
papi_cookie_login(request_rec * r, papi_dir_config * d)
{
	int             valid_date;
	char           *aurl = papi_uri_get_arg(r->pool, r->args, "AURL");
	char           *rurl = papi_uri_get_arg(r->pool, r->args, "RURL");
...
static int 
papi_cookie_logout(request_rec * r, papi_dir_config * d)
{
	int             init = UNSET;
	char           *lcook = papi_test_lcook(r, d, &init);
	char           *aurl = papi_uri_get_arg(r->pool, r->args, "AURL");
...
static int 
papi_cookie_check(request_rec * r, papi_dir_config * d)
{
	int             init = UNSET;
	char           *lcook = papi_test_lcook(r, d, &init);
...
static int 
papi_cookie_checked(request_rec * r, papi_dir_config * d)
{
	int             valid_date;
	char           *code;
...
static int 
papi_send_file(request_rec * r, const char *filename)
{
...
static int 
papi_send_file_with_cookies(request_rec * r, const char *filename, const char *lcook)
{
...
int 
papi_redirect_with_cookies(request_rec * r, const char *uri, const char *lcook)
{
...
static int 
papi_redirect_error_url(request_rec * r, papi_dir_config * d, const char *uri, const char *data)
{
...
int 
papi_redirect_url(request_rec * r, const char *uri)
{
	apr_table_set(r->err_headers_out, "Location", uri);
	return HTTP_MOVED_TEMPORARILY;
}
...
static char    *
papi_encrypt_gen_code(request_rec * r, papi_dir_config * d, const char *assert, int valid_date, const char *data)
{
...
static char    *
papi_gen_user_data(request_rec * r, papi_dir_config * d, char *poa, char *code)
{
	int             i;
