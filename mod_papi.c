...
static char    *
papi_generate_lkey(apr_pool_t * p)
{
 ...
 */
static int 
papi_test_pass_url_pattern(request_rec * r, papi_dir_config * d, const char *curl)
{
 ...
 */
static void    *
create_papi_dir_config(apr_pool_t * pool, char *loc)
{
...
static void    *
create_papi_merge_dir_config(apr_pool_t * pool, void *_base, void *_add)
{
...
papi_auth_hook(request_rec * r)
{
	...
	//Check wether the request matches passthrough filters
		if (papi_test_pass_url_pattern(r, d, curl))
		return OK;
	...
	char           *code = NULL;
	if (papi_get_action(r) == PAPI_CHECKED) {
		//Answer from a GPoA
