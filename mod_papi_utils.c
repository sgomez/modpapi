 ...
 */
char           *
papi_md5_base64(request_rec * r, char *str)
{
...
char           *
papi_escape_string(apr_pool_t * p, const char *str)
{
...
char           *
papi_unescape_string(apr_pool_t * p, const char *str)
{
...
char           *
papi_uri_get_arg(apr_pool_t * p, const char *src, const char *name)
{
	const char     *queries;
...
char           *
papi_uri_add_arg(apr_pool_t * pool, const char *uri, const char *key, const char *value)
{
	char           *sep;
	char           *newuri;
...
papi_regex_t   *
papi_regex_new(apr_pool_t * p, const char *pattern)
{
...
int 
papi_regex_match(papi_regex_t * re, const char *string)
{
	int             ovector[3];
...
char           *
papi_regex_replace(apr_pool_t * p, papi_regex_t * re, const char *string, const char *replace)
{
	int             ovector[3];
	int             length = strlen(string);
	int             rc = pcre_exec(re->pcre_re, NULL, string, length, 0, 0, ovector, 3);
...
char          **
papi_string_split(apr_pool_t * p, char *string, char *token, int limit)
{
	if (limit < 1)
		return NULL;
...
char           *
papi_file_stat(request_rec * r, papi_dir_config * d, const char *path, apr_filetype_e type)
{
	apr_finfo_t     finfo;
	apr_status_t    status;
